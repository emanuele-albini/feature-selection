...
def sample_data(
    ...
    **kwargs,
    ...
    if isinstance(X, pd.DataFrame):
        # Safe handling of n
        if safe and n is not None and n > len(X):
            n = len(X)
        if safe and frac is not None and frac > 1:
            frac = 1
        ...
        return np_sample(X, n=n, replace=replace, random_state=random_state, safe=safe, **kwargs)
    else:
...
def np_sample(
    ...
    # Array sampling case
        ...
        if safe and n > len(a):
            n = len(a)
        return a[random_state.choice(np.arange(len(a)), n, replace=replace)]
